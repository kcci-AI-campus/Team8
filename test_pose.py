"""
[스켈레톤 단독 구조] test_pose_only.py

낙상 CNN(best_int8.tflite)을 전혀 쓰지 않고, COCO 사전학습 포즈 모델만으로 판단합니다.
  포즈 모델(전체 프레임) → 사람마다 누움 판단 → 사람별로 시간 누적 → 낙상

판정 규칙 (사람 한 명 = 트랙 하나 기준)
  1) 최근 FALL_WINDOW_FRAMES번의 포즈 판단 중 "누움"이 FALL_CONFIRM_FRAMES번 이상이면 낙상
  2) 포즈가 STAND_RESET_FRAMES번 연속 "안 누움"이면 그 사람의 누적 상태 초기화 (일어남)

test_pose_first.py와 CNN 부분만 빼고 전부 동일합니다 (포즈 판단, 누적 규칙, 움직임 게이팅,
캡처/프리즈). 두 파일의 결과 차이 = 낙상 CNN이 기여한 몫, 이렇게 해석할 수 있도록 맞춰 두었으니
임계값을 바꿀 때는 두 파일에 똑같이 바꿔 주세요.
...
사전 준비
  ...
  2) state_machine.py도 같은 폴더에 둘 것 (HUD 상태 표시용)
  입력 크기/레이아웃(NCHW·NHWC)/양자화는 모델 정보에서 자동으로 읽으므로 따로 맞출 필요 없음

실행: python3 test_pose_only.py
종료: 영상 창에서 q
"""
import os
import time
...
FALL_CONFIRM_FRAMES = 3     # 그중 "누움"이 이 횟수 이상이면 낙상
...
RESULT_DIR = "results_pose_only"    # 구조별로 캡처 폴더를 분리해 비교하기 쉽게
CAPTURE_COOLDOWN_SEC = 5.0
WINDOW_NAME = "fall detection (pose-only)"
...
def main():
    ...
    while cap.isOpened():
        ...
        for (box, score, kpts), trk in zip(people, assigned):
            if USE_MOTION_GATE:
                ...
                if DEBUG:
                    print(f"[DEBUG] #{trk.id} box={box} motion={motion:.1f} "
                          f"{'' if motion >= MOTION_TH else '→ static'}")
                if motion < MOTION_TH:
            ...
            is_fall = lying_count >= FALL_CONFIRM_FRAMES
            ...
            tag = f"#{trk.id} {status} {score * 100:.0f}% L{lying_count}/{FALL_WINDOW_FRAMES}"
            draw_box(frame, box, COLOR[status], tag)

            if DEBUG:
                print(f"[DEBUG] #{trk.id} lying={lying} window={list(trk.history)} → {status}")
